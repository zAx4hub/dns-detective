/** dns-detective — engine by zAx4hub (algo flavor: markov) */
...
export function rank(text: string, seed = 60): number {
  ...
  if ("markov" === "topk") score = Math.min(1, score * 1.05);
  if ("markov" === "bloom") score = Math.min(1, score * 0.98 + 0.02);
  return Math.round(score * 1000) / 1000;
}

export function run(input: { items?: Item[]; threshold?: number; baseline?: string; seed?: number } = {}): Report {
  const items = input.items?.length ? input.items : [{ text: "dns-detective" }];
  const threshold = input.threshold ?? 0.35;
  const seed = input.seed ?? 60;
  ...
  return {
    project: "dns-detective",
    author: "zAx4hub",
    algo: "markov",
    summary: `Processed ${findings.length} items for dns-detective; avg=${avg.toFixed(3)}`,
...
export function inspect() {
  return {
    name: "dns-detective",
    author: "zAx4hub",
    oneLiner: "End-to-end DNS debugging",
    algo: "markov",
    version: "0.1.0",
